...
import { Heart, Mail, Lock, Eye, EyeOff, ArrowLeft, User, Shield, Users, Building2, Check, X } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";

type Role = "admin" | "elderly" | "caretaker" | "ngo";
...
const roles: { id: Role; label: string; icon: React.ElementType; desc: string }[] = [
  ...
  { id: "elderly", label: "Elderly", icon: Heart, desc: "Elderly individual" },
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <Button variant="hero" size="lg" className="w-full" disabled={!selectedRole} onClick={() => setStep(2)}>
                  Continue
                </Button>
              </motion.div>
            )}
              ...
              <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
                  ...
                  <div className="space-y-2">
                    ...
                    {password && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}>
                        ...
                        <p className={`text-xs mt-1 font-medium ${strength.score <= 1 ? "text-destructive" : strength.score <= 2 ? "text-warning" : "text-success"}`}>
                          {strength.label}
                        </p>
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <Button variant="hero" size="lg" className="flex-1" onClick={() => setStep(3)}>Continue</Button>
                  </div>
                </form>
              </motion.div>
            )}
              ...
              <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <div className="space-y-4 mb-8">
                  <div
                    ...
                    className={`p-5 rounded-2xl border-2 cursor-pointer transition-all ${consent1 ? "border-primary bg-primary/5" : "border-border bg-card"}`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground leading-relaxed">
                          I agree to the CareNest Terms of Service, including data processing, role-based access policies, and platform usage guidelines.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div
                    ...
                    className={`p-5 rounded-2xl border-2 cursor-pointer transition-all ${consent2 ? "border-primary bg-primary/5" : "border-border bg-card"}`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground leading-relaxed">
                          I understand that my consent history will be recorded, I can withdraw consent at any time, and my data will be handled according to our privacy policy.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex gap-3">
                  ...
                  <Button variant="hero" size="lg" className="flex-1" disabled={!consent1 || !consent2}>
                    Create Account
                  </Button>
                </div>
              </motion.div>
